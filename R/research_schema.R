...
.litxr_normalize_llm_digest_for_write <- function(digest, ref_id = NULL) {
  ...
  if (identical(version, "v5")) {
    if (is.null(payload$formulas)) payload$formulas <- list()
    if (!is.list(payload$formulas)) {
      stop("LLM digest field `formulas` must be a list.", call. = FALSE)
    }
  }
...
.litxr_postprocess_llm_digest_read <- function(digest) {
  ...
  if (identical(.litxr_llm_digest_schema_version(digest), "v5")) {
    if (is.null(digest$formulas)) digest$formulas <- list()
  }
  digest
}
...
.litxr_validate_formulas <- function(formulas) {
  if (is.null(formulas) || !length(formulas)) return(invisible(TRUE))
  if (!is.list(formulas)) stop("LLM digest field `formulas` must be a list.", call. = FALSE)
  ...
  formula_ids <- character(length(formulas))
  for (i in seq_along(formulas)) {
    formula <- formulas[[i]]
    if (!is.list(formula)) stop("Each `formulas` item must be a named list.", call. = FALSE)
    missing <- setdiff(c("formula_id", "latex", "display_name", "source_location", "symbols", "plain_language_interpretation", "assumptions"), names(formula))
    if (length(missing)) stop("Each `formulas` item is missing fields: ", paste(missing, collapse = ", "), call. = FALSE)

    formula_id <- scalar_text(formula$formula_id)
    latex <- scalar_text(formula$latex)
    display_name <- scalar_text(formula$display_name)
    source_location <- scalar_text(formula$source_location)
    interpretation <- scalar_text(formula$plain_language_interpretation)
    if (anyNA(c(formula_id, latex, display_name, source_location, interpretation))) {
      stop("Each `formulas` item requires non-empty scalar text fields.", call. = FALSE)
    }
    formula_ids[[i]] <- formula_id

    if (!is.list(formula$symbols)) stop("Each `formulas$symbols` value must be a list.", call. = FALSE)
    for (symbol in formula$symbols) {
      if (!is.list(symbol) || length(setdiff(c("symbol", "meaning"), names(symbol)))) {
        stop("Each `formulas$symbols` item must contain `symbol` and `meaning`.", call. = FALSE)
      }
    }
    assumptions <- formula$assumptions
    if (is.list(assumptions)) assumptions <- unlist(assumptions, use.names = FALSE)
    if (!is.character(assumptions)) stop("Each `formulas$assumptions` value must be a character array.", call. = FALSE)
  }
  if (anyDuplicated(formula_ids)) stop("`formulas$formula_id` values must be unique.", call. = FALSE)
  invisible(TRUE)
}
