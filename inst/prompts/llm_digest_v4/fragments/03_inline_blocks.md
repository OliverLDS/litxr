...
- For research_target_github_links, include a GitHub URL only when the GitHub repository is the paper's research target or artifact, such as a model, framework, system, dataset, benchmark, mechanism, package, or implementation introduced, studied, or evaluated by the paper. Do not include incidental citations or background links. Add category_tags describing what the GitHub repository is about.
