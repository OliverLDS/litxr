...
usage <- function() {
  cat(paste(
    "Usage:",
    "  Rscript scripts/build_literature_graph.R [--upward-ref-ids ID1,ID2] [--downward-ref-ids ID3] [--include-edge-types foundation,extension] [--max-depth 2] [--max-nodes 100] [--output graph.json]",
    "",
    "Options:",
    "  --ref-ids LIST    Bare ids traced in both directions.",
    ...
    "  --include-edge-types LIST  Retain only foundation, extension, comparison, or context edges.",
    "  --exclude-edge-types LIST  Remove foundation, extension, comparison, or context edges.",
    ...
    "  - --ref-ids traces both anchored references and local citing papers.",
...
parse_args <- function(args) {
  out <- list(help = FALSE, ref_ids = NULL, upward_ref_ids = NULL, downward_ref_ids = NULL, include_edge_types = NULL, exclude_edge_types = NULL, max_depth = "2", max_nodes = "100", output = NULL, config = NULL)
  i <- 1L
  while (i <= length(args)) {
    key <- args[[i]]
...
tryCatch({
  ...
  include_edge_types <- parse_ref_ids(parsed$include_edge_types)
  exclude_edge_types <- parse_ref_ids(parsed$exclude_edge_types)
  if (!length(ref_ids) && !length(upward_ref_ids) && !length(downward_ref_ids)) {
    stop("Supply --ref-ids, --upward-ref-ids, or --downward-ref-ids.", call. = FALSE)
  }
