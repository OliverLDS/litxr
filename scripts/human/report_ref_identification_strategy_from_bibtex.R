...
usage <- function() {
  cat(
    paste(
      "Usage:",
      "  Rscript scripts/human/report_ref_identification_strategy_from_bibtex.R --bibtex PATH [--output PATH]",
      "  Rscript scripts/human/report_ref_identification_strategy_from_bibtex.R --ref-ids ID1,ID2 [--output PATH]",
      "  Rscript scripts/human/report_ref_identification_strategy_from_bibtex.R PATH [--output PATH]",
      "",
      "Options:",
      ...
      "  - Generates a markdown file listing ref_id and Identification Strategy from the latest research schema.",
...
if (identical(input_mode, "bibtex")) {
  ...
  if (is.null(parsed$output) || !nzchar(trimws(parsed$output))) {
    output_path <- paste0(tools::file_path_sans_ext(bibtex_path), ".identification_strategy.md")
  } else {
  ...
  if (is.null(parsed$output) || !nzchar(trimws(parsed$output))) {
    output_path <- "report_ref_identification_strategy.md"
  } else {
    output_path <- path.expand(parsed$output)
  }
...
for (entry in resolved_entries) {
  ...
  ident_text <- if (!is.null(digest)) scalar_methods(digest$identification_strategy) else character()
  ...
  report_lines <- c(
    ...
    sprintf("- Identification Strategy: %s", if (!is.na(ident_text) && nzchar(ident_text)) ident_text else "[missing]"),
    ""
  )
}
