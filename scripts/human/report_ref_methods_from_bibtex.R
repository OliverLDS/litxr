...
usage <- function() {
  cat(
    paste(
      "Usage:",
      "  Rscript scripts/human/report_ref_methods_from_bibtex.R --bibtex PATH [--output PATH]",
      "  Rscript scripts/human/report_ref_methods_from_bibtex.R --ref-ids ID1,ID2 [--output PATH]",
      "  Rscript scripts/human/report_ref_methods_from_bibtex.R PATH [--output PATH]",
      "",
      "Options:",
      ...
      "",
      "Behavior:",
      "  - Reads each BibTeX entry, resolves it to a local litxr reference when possible.",
      ...
      "  - Generates a markdown file listing ref_id and Methods from the latest research schema.",
...
methods_text <- function(x) {
  ...
  if (is.list(x) && !is.character(x)) {
    vals <- vapply(x, function(item) {
      if (is.list(item)) {
        if ("method" %in% names(item)) return(scalar_text(item$method))
        if ("name" %in% names(item)) return(scalar_text(item$name))
        if ("text" %in% names(item)) return(scalar_text(item$text))
      }
      scalar_text(item)
    }, character(1))
  } else {
...
diag_state <- litxr_diag_init(
  "scripts/human/report_ref_methods_from_bibtex.R",
...
if (identical(input_mode, "bibtex")) {
  ...
  if (is.null(parsed$output) || !nzchar(trimws(parsed$output))) {
    output_path <- paste0(tools::file_path_sans_ext(bibtex_path), ".methods.md")
  } else {
  ...
  if (is.null(parsed$output) || !nzchar(trimws(parsed$output))) {
    output_path <- "report_ref_methods.md"
  } else {
    output_path <- path.expand(parsed$output)
  }
...
for (entry in resolved_entries) {
  ...
  methods <- if (!is.null(digest)) methods_text(digest$methods) else character()
