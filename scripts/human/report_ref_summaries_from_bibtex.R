...
diag_state <- litxr_diag_init(
  "scripts/human/report_ref_summaries_from_bibtex.R",
...
diag_state <- litxr_diag_step(
  diag_state,
  "read_inputs",
  if (identical(input_mode, "bibtex")) config_started else Sys.time(),
  inputs = list(
    if (identical(input_mode, "bibtex")) litxr_diag_file_meta(bibtex_path) else list(ref_ids = entries$keys)
  ),
  outputs = list(
    list(entry_count = length(entries$entries), input_mode = input_mode)
  )
)
...
diag_state <- litxr_diag_step(
  diag_state,
  "resolve_refs_and_load_digests",
  Sys.time(),
  inputs = list(list(resolved_ref_ids = resolved_ref_ids)),
  outputs = list(list(digest_count = length(digest_map), missing_count = length(unique(missing))))
)
